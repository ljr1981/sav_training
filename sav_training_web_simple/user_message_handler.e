...
feature -- Access
	...
	response (a_request: WSF_REQUEST): WSF_RESPONSE_MESSAGE
		...
		do
			if attached {WSF_STRING} a_request.path_parameter ("user") as al_user then
				l_username := html_decoded_string (al_user.value)
				if a_request.is_request_method ("GET") then
					Result := user_message_get (l_username, a_request)
				elseif a_request.is_request_method ("POST") then
					Result := user_message_response_post (l_username, a_request)
				else
	...
	missing_argument_response (a_message: READABLE_STRING_8; a_request: WSF_REQUEST): WSF_PAGE_RESPONSE
		do
			create Result.make
			Result.set_status_code ({HTTP_STATUS_CODE}.bad_request)
			Result.put_string (a_request.request_uri + ": " + a_message)
		end
